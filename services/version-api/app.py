...
async def auth(request: Request):
    ...
    AUTH_PORT = os.getenv("AUTH_API_PORT", 5201)
...
async def user_details(request: Request):
    ...
    USER_DETAILS_API_PORT = os.getenv("AUTH_API_PORT", 5201)

    url = f"http://{USER_DETAILS_API_HOST}:{USER_DETAILS_API_PORT}/api/user-details/version"
...
if __name__ == "__main__":
    ...
    uvicorn.run(app, host="0.0.0.0", port=int(os.getenv("VERSION_API_PORT", 5200)))
